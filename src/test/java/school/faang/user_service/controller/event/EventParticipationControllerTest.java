...
class EventParticipationControllerTest {
    ...
    void testRegisterParticipant() {
        ...
        Mockito.verify(eventParticipationService, Mockito.times(1)).registerParticipant(EVENT_ID, USER_ID);
    }

    @Test
    void testUnregisterParticipant() {
        eventParticipationController.unregisterParticipant(EVENT_ID, USER_ID);
        Mockito.verify(eventParticipationService, Mockito.times(1)).unregisterParticipant(EVENT_ID, USER_ID);
    }

    @Test
    void testGetParticipant() {
        eventParticipationController.getParticipant(EVENT_ID);
        Mockito.verify(eventParticipationService, Mockito.times(1)).getParticipant(EVENT_ID);
    }

    @Test
    void testGetParticipantCount() {
        eventParticipationController.getParticipantCount(EVENT_ID);
        Mockito.verify(eventParticipationService, Mockito.times(1)).getParticipantsCount(EVENT_ID);
    }
}